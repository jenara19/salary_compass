...
class TestHealthScaling:
    ...
    def test_rotterdam_health_not_scaled_from_madrid_zero(self):
        ...
        madrid_expenses = {
            "rent_2bed": 1200, "utilities": 150, "health_extra": 0,
...
class TestScalingRatio:
    def test_rent_amsterdam_uses_yaml_comfortable(self):
        ...
        assert rent_eur > 1500, f"Amsterdam rent YAML comfortable should exceed €1500, got €{rent_eur}"

    def test_rent_note_indicates_market_anchored(self):
        """rent_2bed note must say YAML/market-anchored."""
        expenses = {
            "rent_2bed": 100, "utilities": 150, "health_extra": 80,
            "groceries_2pax": 600, "transport_2pax": 50, "eating_out": 180,
            "leisure": 100, "misc": 200, "travel": 350, "personal": 700,
        }
        result = scale_expenses_to_city(expenses, "madrid", "amsterdam", {}, pax=2)
    ...
    def test_groceries_ratio_scales_up_madrid_to_amsterdam(self):
        ...
        assert result_amsterdam["items"]["groceries_2pax"]["value_eur"] >= \
...
class TestCalculateBudget:
    @pytest.mark.parametrize("city,scenario", [
        ("madrid", "comfortable"), ("rotterdam", "comfortable"), ("zurich", "comfortable"),
        ("amsterdam", "frugal"), ("london", "generous"), ("berlin", "comfortable"),
        ("oslo", "comfortable"), ("munich", "comfortable"),
    ])
...
class TestYamlValidity:
    CITY_SLUGS = [
        "madrid", "barcelona", "amsterdam", "rotterdam", "berlin",
        "munich", "london", "manchester", "oslo", "zurich", "geneva",
    ]
...
class TestInfrastructureScaling:
    ...
    FRUGAL_TRANSPORT_MADRID = {
        "rent_2bed": 1200, "utilities": 150, "health_extra": 80,
        "groceries_2pax": 600, "transport_2pax": 10,
        "eating_out": 180, "leisure": 100, "misc": 200,
        "travel": 350, "personal": 700,
    }

    FRUGAL_UTILITIES_MADRID = {
        "rent_2bed": 1200, "utilities": 20,
        "health_extra": 80, "groceries_2pax": 600, "transport_2pax": 50,
        "eating_out": 180, "leisure": 100, "misc": 200,
        "travel": 350, "personal": 700,
    }
...
class TestNewcomerSensitivity:
    ...
    BASE = {
        "rent_2bed": 1200, "utilities": 150, "health_extra": 80,
        "groceries_2pax": 600, "transport_2pax": 140, "eating_out": 300,
        "leisure": 150, "misc": 200, "travel": 350, "personal": 700,
    }

    def test_eating_out_higher_when_just_arrived(self):
        """eating_out at settling=0 must be higher than at settling=1."""
        arrived  = scale_expenses_to_city(self.BASE, "madrid", "rotterdam", {}, pax=2, settling_factor=0.0)
        settled  = scale_expenses_to_city(self.BASE, "madrid", "rotterdam", {}, pax=2, settling_factor=1.0)
        assert arrived["items"]["eating_out"]["value_eur"] > settled["items"]["eating_out"]["value_eur"], (
    ...
    def test_eating_out_max_premium_is_25_pct(self):
        ...
        arrived = scale_expenses_to_city(self.BASE, "madrid", "rotterdam", {}, pax=2, settling_factor=0.0)
        settled = scale_expenses_to_city(self.BASE, "madrid", "rotterdam", {}, pax=2, settling_factor=1.0)
        ratio = arrived["items"]["eating_out"]["value_eur"] / settled["items"]["eating_out"]["value_eur"]
    ...
    def test_groceries_max_premium_is_10_pct(self):
        ...
        arrived = scale_expenses_to_city(self.BASE, "madrid", "rotterdam", {}, pax=2, settling_factor=0.0)
        settled = scale_expenses_to_city(self.BASE, "madrid", "rotterdam", {}, pax=2, settling_factor=1.0)
        ratio = arrived["items"]["groceries_2pax"]["value_eur"] / settled["items"]["groceries_2pax"]["value_eur"]
    ...
    def test_settling_factor_50_is_between_0_and_100(self):
        ...
        arrived = scale_expenses_to_city(self.BASE, "madrid", "rotterdam", {}, pax=2, settling_factor=0.0)
        half    = scale_expenses_to_city(self.BASE, "madrid", "rotterdam", {}, pax=2, settling_factor=0.5)
        settled = scale_expenses_to_city(self.BASE, "madrid", "rotterdam", {}, pax=2, settling_factor=1.0)
