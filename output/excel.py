...
def generate_excel_report(
    ...
    F = {
        "title":      _fmt(bold=True, font_size=14, font_color=_C_HEADER_FG,
                           bg_color=_C_HEADER_BG, border=0),
        "subtitle":   _fmt(bold=True, font_size=10, font_color=_C_SUBHEAD_FG,
                           bg_color=_C_SUBHEAD_BG, border=0),
        "header":     _fmt(bold=True, font_size=9, font_color=_C_HEADER_FG,
                           bg_color=_C_SUBHEAD_BG, align="center", border=0),
        ...
        "surplus_pos":_fmt(font_size=9, num_format='€#,##0', bold=True,
                           font_color=_C_SURPLUS_POS),
        "surplus_neg":_fmt(font_size=9, num_format='€#,##0', bold=True,
                           font_color=_C_SURPLUS_NEG),
        "pct":        _fmt(font_size=9, num_format='0.0%'),
        "amber":      _fmt(font_size=9, bold=True, font_color=_C_AMBER,
                           bg_color=_C_AMBER_BG),
        "amber_money":_fmt(font_size=9, bold=True, num_format='€#,##0',
                           font_color=_C_AMBER, bg_color=_C_AMBER_BG),
    ...
    _write_trajectory(wb, F, city_inputs, results_matrix, scenarios_def, city_names, scen_names, slugs)
    _write_negotiation(wb, F, city_inputs, results_matrix, scenarios_def, city_names,
                       scen_names, slugs, home_city_slug)
    _write_assumptions(wb, F, city_inputs, city_names, scen_names, scenarios_def,
                       col_inflation_rate, home_city_slug)
...
def _write_summary(wb, F, city_inputs, results_matrix, scenarios_def,
                   city_names, scen_names, slugs):
    ...
    for row_i, scen in enumerate(scenarios_def):
        ...
        for j, slug in enumerate(slugs):
            ...
            ws.write_number(row, col + 2, sur,
                            F["surplus_pos"] if sur >= 0 else F["surplus_neg"])

    # Perks note
    has_perks = any(ci.get("perks_monthly_eur", 0) > 0 for ci in city_inputs.values())
    note_row = 3 + len(scenarios_def) + 1
    if has_perks:
        ws.merge_range(note_row, 0, note_row, num_data_cols,
                       "* Net includes tax-free perks (travel allowance + meal vouchers)", F["caption"])
    ws.merge_range(note_row + 1, 0, note_row + 1, num_data_cols,
                   "Directional estimates only — not financial or tax advice.", F["caption"])
...
def _write_budget(wb, F, city_inputs, results_matrix, scenarios_def,
                  city_names, scen_names, slugs):
    ...
    ws.merge_range(0, 0, 0, len(slugs),
                   f"SalaryCompass — Budget Breakdown  [{date.today()}]", F["title"])
    ws.set_row(0, 22)
    ...
    for scen in scenarios_def:
        ...
        for row_i, cat in enumerate(cats):
            ...
            for j, slug in enumerate(slugs):
                ...
                val = items.get(cat, {}).get("eur", 0) if isinstance(items.get(cat), dict) \
                      else items.get(cat, 0)
                ws.write_number(current_row, j + 1, val,
                                F["money_alt"] if alt else F["money"])
            current_row += 1
...
def _write_trajectory(wb, F, city_inputs, results_matrix, scenarios_def,
                      city_names, scen_names, slugs):
    ...
    ws.merge_range(0, 0, 0, 7,
                   f"SalaryCompass — 10-Year Trajectory  [{date.today()}]", F["title"])
    ws.set_row(0, 22)

    COL_HEADS = ["Yr", "Gross (local)", "Net €/mo", "Expenses €/mo",
                 "Surplus €/mo", "Cumulative €", "Eff. rate", "Events"]
    ...
    for slug in slugs:
        ...
        for scen in scenarios_def:
            # Block header
            ws.merge_range(current_row, 0, current_row, 7,
                           f"{city_names[slug]}  ·  {scen['name']}", F["subtitle"])
            ...
            for row_i, yr in enumerate(traj):
                ...
                sur_fmt = (F["amber_money"] if has_event else
                           (F["surplus_pos"] if sur >= 0 else F["surplus_neg"]))
...
def _write_negotiation(wb, F, city_inputs, results_matrix, scenarios_def,
                       city_names, scen_names, slugs, home_city_slug):
    ...
    ws.merge_range(0, 0, 0, 5,
                   f"SalaryCompass — Negotiation Targets  [{date.today()}]", F["title"])
    ws.set_row(0, 22)
    ...
    if home_city_slug in city_inputs and home_city_slug in results_matrix:
        ...
        home_net = (results_matrix[home_city_slug][ref_scen]["net"]["net_monthly_eur"]
    ...
    row = 1
    ws.write(row, 0, f"Reference city: {city_names.get(home_city_slug, home_city_slug)}", F["label"])
    ...
    for slug in non_home:
        ...
        for scen in scenarios_def:
            ...
            be_gross = find_target_gross(target_net, ci["country_code"],
                                         active_schemes=list(ci["active_schemes"]))
            from engine.tax import calculate_net
            check_net = calculate_net(be_gross, ci["country_code"],
                                      list(ci["active_schemes"]))["net_monthly_eur"] + perks
        ...
        from engine.tax import calculate_net as _cnet
        ...
        mandatory_ot = sum(h.get("one_time", 0) * eur_rate
                           for h in hidden if h.get("mandatory") and "one_time" in h)
        comfortable_budget = results_matrix[slug][scen_names[min(1, len(scen_names)-1)]]["budget"]["total_eur"]
        ...
        if permit:
            ...
            ws.write(row, 2, f"EU/EEA: {permit.get('eu_eea', {}).get('timeline', 'n/a')}", F["normal"])
            ws.write(row, 3, f"Non-EU: {permit.get('non_eu', {}).get('timeline', 'n/a')}", F["normal"])
            row += 1
...
def _write_assumptions(wb, F, city_inputs, city_names, scen_names, scenarios_def,
                        col_inflation_rate, home_city_slug):
    ...
    ws.merge_range(0, 0, 0, 3,
                   f"SalaryCompass — Assumptions & Inputs  [{date.today()}]", F["title"])
    ws.set_row(0, 22)
