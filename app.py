...
@st.cache_data
def cached_scale_expenses(
    user_expenses_tuple, home_city_slug, target_city_slug, pax, settling_factor
):
    return scale_expenses_to_city(
        dict(user_expenses_tuple),
        home_city_slug,
        target_city_slug,
        {},
        pax=pax,
        ...
        for cat, item in base["items"].items():
            ...
            if cat not in city_overrides:
                alias = (
                    "health_kvg"
                    if cat == "health_extra"
                    else ("health_extra" if cat == "health_kvg" else None)
                )
            ...
            if override_key in city_overrides:
                ...
                yaml_comfortable = (
                    city_col.get(cat, {}).get("comfortable", 0)
                    or city_col.get(override_key, {}).get("comfortable", 0)
                    or 0
                )
                scaled_eur[cat] = (
                    yaml_comfortable * eur_rate * city_overrides[override_key] * s_mult
                )
            else:
...
with st.sidebar:
    ...
    current_country = st.selectbox(
        "Current country of residence", SUPPORTED_COUNTRIES, key="current_country"
    )
    ...
    children = st.number_input(
        ...
        min_value=0,
        max_value=10,
        value=st.session_state.get("user_children", 0),
        key="user_children",
    )
    ...
    st.divider()
    st.caption(
        "📋 All figures are directional estimates. Consult a tax adviser before making decisions."
    )
...
with tab_setup:
            ...
            for j, (city_slug, col) in enumerate(zip(chunk, city_cols, strict=False)):
                ...
                with col:
                    ...
                    travel_allowance = st.number_input(
                        ...
                        min_value=0,
                        max_value=round(5000 / max(country_data["eur_rate"], 0.001)),
                    ...
                    meal_vouchers = st.number_input(
                        ...
                        min_value=0,
                        max_value=round(2000 / max(country_data["eur_rate"], 0.001)),
                    ...
                    # ── Net income preview ──────────────────────────────────
                    _net = cached_calculate_net(
                        effective_gross, country_code, tuple(active_schemes)
                    )
                    ...
                    with st.expander("🔧 Override city cost estimates"):
                        ...
                        _override_cats = [
                            ...
                            (
                                ...
                                "🏥 Health ins.",
                                0,
                                round(4000 / _eur_rate),
                            ),
                            (
                                "groceries_2pax",
                                "🛒 Groceries",
                                0,
                                round(3000 / _eur_rate),
                            ),
                            (
                                "transport_2pax",
                                "🚌 Transport",
                                0,
                                round(1000 / _eur_rate),
                            ),
    ...
    if not selected_city_slugs:
        ...
        # ── Detailed mode ──────────────────────────────────────────────────────
            ...
            with _header_cols[0]:
                st.caption("Category")
            with _header_cols[1]:
                st.caption("Your spend (€/mo)")
            with _header_cols[2]:
                st.caption("City estimate")
            with _header_cols[3]:
                st.caption("Diff")
            ...
            user_expenses_by_cat = {
                cat: st.session_state.get(f"exp_{cat}", 0) for cat in UI_CAT_KEYS
            }
        ...
        with st.expander("🎯 Lifestyle anchors — portable expenses"):
            ...
            pets = st.number_input(
                "Monthly pets (€)", min_value=0, step=10, value=0, key="anchor_pets"
            )
            streaming = st.number_input(
                ...
                step=5,
                value=30,
                key="anchor_streaming",
            )
            other_pa = st.number_input(
                "Monthly other personal recurring (€)",
                min_value=0,
                step=10,
                value=0,
                key="anchor_other",
    ...
    if not selected_city_slugs:
        ...
        _anchor_note = (
            f"  ·  Lifestyle anchors: **€{_lifestyle_total:,}**  ·  Total baseline: **€{_baseline_total:,}**"
            if _lifestyle_total
            else ""
        )
...
with tab_compare:
    if not city_inputs:
        ...
        with r_tab1:
            ...
            st.dataframe(
                overview_df.style.apply(_style_overview, axis=None).format("€{:,.0f}"),
                width="stretch",
            )
            ...
            for key in all_item_keys:
                ...
                for slug in city_inputs:
                    ...
                    if item:
                        lv = item["value_local"]
                        ev = item["value_eur"]
                        row[city_names[slug]] = f"{ccy} {lv:,}" + (
                            f" (€{ev:,})" if ccy != "EUR" else ""
                        )
                    else:
                        row[city_names[slug]] = "—"
                rows.append(row)
            ...
            for slug in city_inputs:
                ...
                ccy = b["currency"]
                total_row[city_names[slug]] = (
                    f"**{ccy} {b['total_local']:,}** (€{b['total_eur']:,})"
                )
            ...
            if rate_change_entries:
                ...
                for rc_e in rate_change_entries:
                    ...
                    with st.container():
                        ...
                        c1.metric(
                            f"Net in Year {rc_e['traj_yr'] - 1} ({rc_e['old_pct']}% ruling)",
                            f"€{rc_e['net_before']:,}/mo",
                        )
                        c2.metric(
                            f"Net in Year {rc_e['traj_yr']} ({rc_e['new_pct']}% ruling)",
                            f"€{rc_e['net_after']:,}/mo",
                            delta=f"-€{rc_e['rc_drop']:,}/mo",
                            delta_color="inverse",
                        )
                        c3.metric(
                            "Gross needed to keep Year N net",
        ...
        with r_tab3:
            ...
            if len(city_inputs) < 2:
                ...
                ref_city_name = st.selectbox(
                    "Reference city", city_name_list, index=0, key="ref_city"
                )
                ref_scen_name = st.selectbox(
                    "Reference scenario", scen_names, index=1, key="ref_scen"
                )
                ...
                comfortable_budget_eur = results_matrix[target_slug][
                ...
                if permit:
                    user_is_eu = (
                        target_country_d.get("currency") == "EUR" or True
                    )  # show both paths
        ...
        with r_tab4:
            ...
            for city_slug in city_inputs:
                ...
                with st.expander(f"📍 {city_names[city_slug]}"):
                    ...
                    st.markdown(
                        f"**Country disclaimer:** {country.get('disclaimer', 'Directional estimates only.')}"
                    )
                    ...
                    st.markdown(
                        f"**Healthcare model:** {hc.get('model', 'Unknown')} — {hc.get('note', '')}"
                    )
            ...
            if lifestyle_anchors_eur:
                ...
                anchor_df = pd.DataFrame(
                    ...
                    + [
                        {
                            "Category": "**Total**",
                            "EUR/month": sum(lifestyle_anchors_eur.values()),
                        }
                    ]
                )
            ...
            if lifestyle_anchors_eur:
                st.markdown(
                    f"- **Lifestyle anchors total:** €{sum(lifestyle_anchors_eur.values()):,.0f}/month"
                )
